...
const seleccionAlbum = async (id) => {
    ...
    if (peticion !== "error") {
        ...
        <img src="${peticion.images.find(imagen => imagen.height === 640)?.url}" alt="">
            ...
            const tracks = peticion.tracks.items.map((track,index)=>{
                ...
                <a data-playlist data-title="${index+1}. ${track.name}" data-artist="${artists}" data-img="http://dmitryvolkov.me/demo/volna/img/covers/cover.svg" href="https://dmitryvolkov.me/demo/blast2.0/audio/12071151_epic-cinematic-trailer_by_audiopizza_preview.mp3" class="single-item__cover">
                ...
                <div class="single-item__title">
                    <h4><a href="#">${index+1}. ${track.name}</a></h4>
                    <span><a href="#">${artists}</a></span>
                </div>
                
                <span class="single-item__time">${milisegundosAminutosYsegundos(track.duration_ms)}</span>
            </li>`
            }).join("")
...
const milisegundosAminutosYsegundos =(millis) => {
    var minutos = Math.floor(millis / 60000);
    var segundos = ((millis % 60000) / 1000).toFixed(0);
